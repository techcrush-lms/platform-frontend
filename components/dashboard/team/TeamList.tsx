...
const TeamList = ({
  ...
  const renderTableBody = (members: typeof invites, noText: string) => (
    <tbody className='divide-y divide-gray-100 dark:divide-gray-700'>
      {members.map((member, index) => (
        ...
        >
            ...
            >
              ...
              >
                <span className='font-medium text-gray-800 dark:text-gray-100 truncate'>
                  {member.user?.id === profile?.id ? 'You' : member.name || '-'}
                </span>
                {member.user?.id !== profile?.id && <PencilIcon size='13' />}
              </div>
            </Link>
          </td>
          ...
          <td className='px-4 py-2'>
            <span
              className={`font-medium px-2 py-1 rounded-full ${
                member.status === ContactInviteStatus.ACTIVE
      ...
      {!members.length && <TableEndRecord colspan={5} text={noText} />}
    </tbody>
  );
  ...
  return (
    <Tabs defaultValue='business-administrator'>
      ...
      <TabsContent value='business-administrator'>
