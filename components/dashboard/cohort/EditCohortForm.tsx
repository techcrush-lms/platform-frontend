...
import useCohort from '@/hooks/page/useCohort';
...
import { UpdateCohortSchema } from '@/lib/schema/cohort.schema';
import {
  CreateTrackSchema,
  UpdateTrackSchema,
} from '@/lib/schema/product.schema';
import { brandPreffix } from '@/lib/utils';
import { updateCohort } from '@/redux/slices/cohortSlice';
...
const EditCohortForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error, value } = UpdateCohortSchema.validate(body);
  ...
  // Update form state when cohort data is fetched
  useEffect(() => {
    if (cohort) {
      setBody({
        name: cohort.name,
        cohort_number:
          split(cohort.cohort_number, '-')?.[1] || cohort.cohort_number,
        description: cohort.description!,
        cohort_month: cohort.cohort_month,
        cohort_year: cohort.cohort_year,
        multimedia_id: cohort.multimedia?.id!,
        group_link: cohort.group_link,
      });

      setImagePreview(cohort.multimedia?.url!);
    }
  }, [cohort]);
    ...
    <ThemeDiv className='mt-6 p-6'>
      ...
      <div className='mb-6'>
        ...
        <p className='text-sm text-muted-foreground'>
          Edit cohort used to structure and organize courses.
        </p>
      </div>
